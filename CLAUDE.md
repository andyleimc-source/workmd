...
> **机械前提**:任务必须挂在某个项目下——属于现有项目就挂它,零散小事挂 `P00-misc`;建时一律走 `scripts/new-task.sh <slug> [P0X 或项目目录名]`(省略第二参数默认落 `P00-misc`)。**别手动建目录、别自己编编号。** 下面讲的是「该不该建、往哪儿建、怎么告诉用户」。

### 分权:任务可自动建,项目永不自动建

- **项目(P)永不自动建。** 只有两条路:① 用户主动说要建;② AI 识别到「像项目了」(`P00-misc` 下同主题任务变多 / 跨周跨月主题)→ **征询同意**才建。很多东西其实只是小任务,不要擅自升格。
- **任务(T)可以按下面门槛自动建**,不必每次都等用户开口。
- **升格通道**:默认先落成任务兜着(挂现有项目或 `P00-misc`);某天同主题任务变多 / 用户说它重要 → AI **提议**升格为项目,**要用户点头**,不自动。

### 什么时候判断(不开子进程,省 token)

判断逻辑是 AI **正常推理的一部分**,不起独立进程、不每轮跑子 agent:
- **平时默默攒信号、不打断**:后台留意信号,攒到高置信度才开口,别在用户专注干活时插话。
- **收工兜底**:用户说 收工/88/再见/落盘 等结束语时,回头判断这轮该不该落成任务/项目。
- **幂等去重**:一轮对话里同一件事只落一次盘。若前面已经建过(并已告诉用户),收工时**只补进度 + 更新 handoff,不重复建**。

### 任务门槛(偏紧,但主动问)

信号:①明确目标 ②多步动作 ③跨会话可能 ④有产出物(文件/链接/决策)。
- **自动建**:≥3 个信号 **且** 归属清楚(能挂现有项目或明确进 `P00-misc`)。
- **问一句**:信号临界(正好 2 个),或归属不清 →
  > 这看起来是个任务,要挂哪个项目下?(零散小事就放 `P00-misc`)slug 建议:`xxx`
- **不建**:≤1 个(纯问答 / 查资料 / 一次性小改)——连问都不问。
- 取舍:**偏紧**——拿不准宁可问一句,也不擅自堆卡。堆废卡的清理成本,比补问一句高得多。

### 归属判断(凡落盘必亮去向)

主信号是用户有没有报项目名,内容匹配只是备胎:
1. **报了项目名**(「继续/进行 P0X」)→ 归属确定,直接建在那,不猜。
2. **没报但强命中**现有项目(提到它独有的文件/产物/关键词)→ **补问一句**「这是接着 P0X 干吗?」。
3. **没报也没命中** → 默认挂 `P00-misc`,不反复纠结。

> **凡落盘必当场说明去向**:无论新建任务、归到已有项目、还是升格项目,都明说「我把这个落成 T0X / 挂到 P0X 了」,绝不闷声建。误挂成本远高于多说一句。

> **过滤器还在,只是换了形态。** 老规矩靠「事前一律拦下等人点头」防知识库长满空壳;现在靠**任务门槛偏紧 + 项目永不自动建 + 每次落盘都亮去向**——高置信度的任务 AI 替你兜住,拿不准的照样问,项目这种重决策永远等你点头。透明 + 克制,而不是干脆全都不动手。
...
1. **项目不自动建、不自动归档;任务按门槛(§三)可自动建,但凡落盘必亮去向。** 项目和归档这两件重决策,提醒可以、代劳不行;任务这种轻决策,高置信度替用户兜住,拿不准就问一句。
