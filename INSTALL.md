...
python3 --version && perl --version >/dev/null && echo "✅ python3 / perl 都在（脚本依赖）"
```
...
跑一次冒烟测试,确认建/归档链路通:
```bash
./scripts/new-task.sh install-smoke-test && ./scripts/detect-done-projects.sh; echo "（无输出=正常，新库没有停滞项目）"
```
测完把这个测试任务删掉(`rm -rf projects/P00-misc/tasks/T01-*`),并把 `assets/codes.md` 恢复原样(删掉那一行、「下一个可用」改回 `T01`),然后告诉用户「链路通了,测试痕迹已清干净」。
